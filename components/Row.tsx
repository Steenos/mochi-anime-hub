...
function Row({ title, shows }: Props) {
  ...
  return (
    <div className=" h-40 space-y-0.5 md:space-y-2  ">
      <h2
        className="w-56 cursor-pointer text-sm font-semibold text-gray-200 transition
        duration-200 hover:text-white md:text-2xl"
      >
        {title}
      </h2>

      <div className="group relative z-0 md:-ml-2">
        <ChevronLeftIcon
        ...
        <div
          ref={rowRef}
          className=" relative z-0 flex items-center space-x-0.5 overflow-x-scroll scrollbar-hide md:space-x-2.5 md:p-2"
        >
          {/* thumbnail */}
          {shows.map((show) => (
            <Thumbnail key={show.mal_id} show={show} />
          ))}
        </div>
